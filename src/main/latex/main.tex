...
Ebenfalls wird eine explizite Typkonversation (nachfolgend casting gennant) implementiert.
...
Innerhalb der brakets befindet sich der Zieldatentyp: \texttt{[TYPE]}
...
Der Literal zwischen \texttt{LBRACKET} und \texttt{RBRACKET} muss vom Datentyp \footnote{int64 oder nat64} sein.
Unsere Erweiterung unterstützt kein casting vom Datentyp \texttt{bool}.
Ein explizites casting zum Typ \texttt{bool} führt zu einem Kompilierungsfehler.
...
\begin{itemize}
    ...
    \item \textbf{Negative Werte}: Wird ein Laufzeitfehler geworfen. Bei einem expliziten casting wird der negati.ve Wert * (-1) gerechnet % TODO --> Soll etwas anderes passieren?
...
\begin{table}[h]
    ...
    \begin{tabular}{rlll}
        \hline
        Quell- \textbackslash \ Zieldatentyp & int64 & nat64 \\ \hline
        int64                   & \cmark        & value >= 0 -> konvertierung, value < 0 -> value * (-1) \cmark *       & \xmark     \\ %TODO wollen wir das so?
