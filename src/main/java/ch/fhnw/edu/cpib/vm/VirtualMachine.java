...
public class VirtualMachine implements IVirtualMachine {
    ...
    private static final String SP_OVER_HP = "Stack pointer over heap pointer.";

    private static final String EP_OVER_HP = "Extreme pointer over heap pointer.";
    ...
    // nat extension
    public class LoadImNatExec extends LoadImNat implements IExecInstr {
        public LoadImNatExec(long value) {
            super(value);
        }
    ...
    public class LoadImBoolExec extends LoadImBool implements IExecInstr {
        public LoadImBoolExec(boolean value) {
            super(value);
        }
    ...
    public class LoadAddrAbsExec extends LoadAddrAbs implements IExecInstr {
        public LoadAddrAbsExec(int absAddress) {
            super(absAddress);
        }
    ...
    // nat extension
    public class NegNatExec extends NegNat implements IExecInstr {
        public void execute() {
            store[sp - 1] = Data.natInv(store[sp - 1]);
            pc = pc + 1;
        }
    }
    ...
    // nat extension
    public class AddNatExec extends AddNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natAdd(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class SubNatExec extends SubNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natSub(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class MultNatExec extends MultNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natMult(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class DivTruncNatExec extends DivTruncNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natDivTrunc(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class ModTruncNatExec extends ModTruncNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natModTrunc(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class DivEuclNatExec extends DivEuclNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natDivEucl(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class ModEuclNatExec extends ModEuclNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natModEucl(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class DivFloorNatExec extends DivFloorNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natDivFloor(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class ModFloorNatExec extends ModFloorNat implements IExecInstr {
        public void execute() throws ExecutionError {
            sp = sp - 1;
            store[sp - 1] = Data.natModFloor(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class EqNatExec extends EqNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natEQ(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class NeNatExec extends NeNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natNE(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class GtNatExec extends GtNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natGT(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class LtNatExec extends LtNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natLT(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class GeNatExec extends GeNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natGE(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }

    // nat extension
    public class LeNatExec extends LeNat implements IExecInstr {
        public void execute() {
            sp = sp - 1;
            store[sp - 1] = Data.natLE(store[sp - 1], store[sp]);
            pc = pc + 1;
        }
    }
    ...
    public class RelJumpExec extends RelJump implements IExecInstr {
        public RelJumpExec(int jumpAddr) {
            super(jumpAddr);
        }
    ...
    public class InputIntExec extends InputInt implements IExecInstr {
        public InputIntExec(String indicator) {
            super(indicator);
        }
    ...
    // nat extension
    public class InputNatExec extends InputNat implements IExecInstr {
        public InputNatExec(String indicator) {
            super(indicator);
        }
    ...
    public class OutputIntExec extends OutputInt implements IExecInstr {
        public OutputIntExec(String indicator) {
            super(indicator);
        }
    ...
    // nat extension
    public class OutputNatExec extends OutputNat implements IExecInstr {
        public OutputNatExec(String indicator) {
            super(indicator);
        }
